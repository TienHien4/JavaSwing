...
public class ThuChiQL {
    ...
    public void themThuChi(ThuChi thuchi){
        FileData.ds_ThuChi.add(thuchi);
        file.writeThuChiToFile();
    }
    
    public boolean xoaThuChi(int index){
        if (FileData.ds_ThuChi.get(index).getLoaichiphi().equalsIgnoreCase("Hóa Đơn") || FileData.ds_ThuChi.get(index).getLoaichiphi().equalsIgnoreCase("Nhập Kho") 
                    || FileData.ds_ThuChi.get(index).getLoaichiphi().equalsIgnoreCase("Trả Lương")){
            return false;
        } else {
            FileData.ds_ThuChi.remove(index);
            file.writeThuChiToFile();
            return true;
        }
    }
